...
private:
  ...
  const static int arrayLength = 18; // 线头绘制范围 - 长
  const static int arrayWidth = 9;   // 宽
  const static int arrayLineHeadLenM2 =
      arrayLength * arrayLength + arrayWidth * arrayWidth;
...
#define OPTIMIZATION 0 // 类型优化，减少switch语句，1开启*9/
  ...
  void angle1EastWest(QPainterPath &linePath, int &sx, int &sy, int &ex,
                      int &ey);
  void angle1SouthNorth(QPainterPath &linePath, int &sx, int &sy, int &ex,
                        int &ey);
#endif
  ...
  void drawLineHead(const ORIENTION o, const LINE_HEAD_TYPE lht, const int x,
                    const int y, QPainter &p, QPainterPath &linePath,
                    QPainterPath &graphPath); // 绘制线头
  void drawStraightLine(int sx, int sy, int ex, int ey, QPainterPath &linePath,
                        QPainterPath &graphPath); // 直连线绘制

public:
  Chart_Line(QWidget *parent = nullptr,
             PaintChartType type = PaintChartType::LINE)
      : Chart_Base(parent, type, false, false, 0, 2) {
    extendWidth45 = extendWidth * sin45Val;
    arrayLength45 = arrayLength * sin45Val;
#if 1
    double tmpRad45AddX =
        qDegreesToRadians(45.0) + qAtan(double(arrayWidth) / arrayLength);
    double tmpRad45SubX =
        qDegreesToRadians(45.0) - qAtan(double(arrayWidth) / arrayLength);
  ...
  Chart_Line(Chart_Line &cr) : Chart_Base(cr) {
    ...
    chartText.textType2->setPalette(
        QPalette(QPalette::Background, QColor(255, 255, 255, 150)));
  }
  Chart_Line(int x1, int y1, int x2, int y2, QWidget *parent = nullptr,
             PaintChartType type = PaintChartType::LINE)
      : Chart_Base(x1, y1, x2, y2, parent, type) {
    chartText.textType2->setAttribute(Qt::WA_StyledBackground, true);
    chartText.textType2->setAutoFillBackground(true);
    chartText.textType2->setPalette(
        QPalette(QPalette::Background, QColor(255, 255, 255, 150)));
  ...
  void setStartChart(Chart_Base *cb) {
    startChartMag = cb;
  }                      // 设置起点指向的图形
  ...
  LINE_HEAD_TYPE &getStartLineHeadType() {
    return startLineHeadType;
  } // 获取起点线头类型
  void setStartLineHeadType(LINE_HEAD_TYPE l) {
    startLineHeadType = l;
    update();
  } // 设置起点线头类型
  LINE_HEAD_TYPE &getEndLineHeadType() {
    return endLineHeadType;
  } // 获取终点线头类型
  ...
  void setStartMagIndex(int i) {
    startMagIndex = i;
  } // 设置起点指向的图形的磁力点索引
  int getStartMagIndex() {
    return startMagIndex;
  } // 获取起点指向的图形的磁力点索引
  void setEndMagIndex(int i) {
    endMagIndex = i;
  } // 设置终点指向的图形的磁力点索引
  int getEndMagIndex() { return endMagIndex; } // 获取终点指向的图形的磁力点索引

  void setStartDirect(ORIENTION direct) {
    startDirect = direct;
  } // 设置连线起点的朝向
  void resetStartDirect() {
    startDirect = ORIENTION::NONE;
  } // 重置连线起点的朝向
  void setEndDirect(ORIENTION direct) {
    endDirect = direct;
  } // 设置连线终点的朝向
  void resetEndDirect() { endDirect = ORIENTION::NONE; } // 重置连线终点的朝向

  friend QDataStream &operator<<(QDataStream &fout,
                                 const Chart_Line &cl); // 重载输出运算符
  friend QDataStream &operator>>(QDataStream &fin,
                                 Chart_Line &cl); // 重载输入运算符

signals:
  void sendLineStyle(QPen &qp, QBrush &qb, LINE_HEAD_TYPE &startLineHeadType,
                     LINE_HEAD_TYPE &endLineHeadType); // 发送连线信息
};
