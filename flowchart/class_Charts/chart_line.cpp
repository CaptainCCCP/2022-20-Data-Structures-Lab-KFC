...
void Chart_Line::drawLineHead(const ORIENTION o, const LINE_HEAD_TYPE lht,
                              const int x, const int y, QPainter &p,
                              QPainterPath &linePath, QPainterPath &graphPath) {
  ...
  case ORIENTION::NORTHEAST: {
    ...
    QPolygonF tmp;
    tmp << QPointF(x + extendWidth45 - (arrayLength45 - arrayLength75),
                   y - extendWidth45 - (arrayWidth75 - arrayLength45))
        << QPointF(x + extendWidth45 + (arrayLength15 - arrayLength45),
                   y - extendWidth45 + (arrayLength45 - arrayWidth15))
        << QPointF(x + (arrayLength15 - arrayLength45),
                   y + (arrayLength45 - arrayWidth15))
        << QPointF(x - (arrayLength45 - arrayLength75),
                   y - (arrayWidth75 - arrayLength45))
        << QPointF(x + extendWidth45 - (arrayLength45 - arrayLength75),
                   y - extendWidth45 - (arrayWidth75 - arrayLength45));
...
void Chart_Line::drawStraightLine(int sx, int sy, int ex, int ey,
  ...
  if (sx < ex && sy < ey) {
    QPolygonF tmp;
    tmp << QPointF(sx, sy) << QPointF(sx + containsWidth1_2, sy)
        << QPointF(ex, ey - containsWidth1_2) << QPointF(ex, ey)
        << QPointF(ex - containsWidth1_2, ey)
        << QPointF(sx, sy + containsWidth1_2) << QPointF(sx, sy);
    graphPath.addPolygon(tmp);
  } else if (sx > ex && sy > ey) {
    QPolygonF tmp;
    tmp << QPointF(ex, ey) << QPointF(ex + containsWidth1_2, ey)
        << QPointF(sx, sy - containsWidth1_2) << QPointF(sx, sy)
        << QPointF(sx - containsWidth1_2, sy)
        << QPointF(ex, ey + containsWidth1_2) << QPointF(ex, ey);
    graphPath.addPolygon(tmp);
  } else if (sx < ex) {
    QPolygonF tmp;
...
void Chart_Line::paintChart(QPainter &p) {
  ...
  chartText.textType2->setGeometry(
      (width() >> 1) - (chartText.textType2->width() >> 1),
      (height() >> 1) - (chartText.textType2->height() >> 1),
      chartText.textType2->width(), chartText.textType2->height());
  int midx, midy;
  int sx, sy, ex, ey;
  QPainterPath linePath;
  ...
  case ORIENTION::NORTH: {
    ...
    drawLineHead(startDirect, startLineHeadType, startPos.rx(), startPos.ry(),
                 p, linePath, *graphPath);
    linePath.moveTo(sx, sy);
    switch (endDirect) {
    case ORIENTION::NORTH: {
      ex = endPos.rx();
      ...
      if (sy > ey) {
        ...
        chartText.textType2->setGeometry(
            (width() >> 1) - (chartText.textType2->width() >> 1),
            ey - (chartText.textType2->height() >> 1),
            chartText.textType2->width() + 30, chartText.textType2->height());
      } else {
        ...
        chartText.textType2->setGeometry(
            (width() >> 1) - (chartText.textType2->width() >> 1),
            sy - (chartText.textType2->height() >> 1),
            chartText.textType2->width() + 30, chartText.textType2->height());
      }
      drawLineHead(endDirect, endLineHeadType, endPos.rx(), endPos.ry(), p,
                   linePath, *graphPath);
    } break;
    case ORIENTION::EAST: {
      ...
      if (sy > ey) {
        if (sx > ex) {
          ...
          linePath.lineTo(ex, ey);
          graphPath->addRect(sx - containsWidth1_2, sy, containsWidth,
                             midy - sy);
          graphPath->addRect(sx, midy - containsWidth1_2, ex - sx,
                             containsWidth);
          graphPath->addRect(ex - containsWidth1_2, midy, containsWidth,
                             ey - midy);
          chartText.textType2->setGeometry(
              (width() >> 1) - (chartText.textType2->width() >> 1),
              midy - (chartText.textType2->height() >> 1),
              chartText.textType2->width() + 30, chartText.textType2->height());
        }
      } else {
        if (sx > ex) {
          ...
          linePath.lineTo(ex, ey);
          graphPath->addRect(sx, sy - containsWidth1_2, midx - sx,
                             containsWidth);
          graphPath->addRect(midx - containsWidth1_2, sy, containsWidth,
                             ey - sy);
          graphPath->addRect(midx, ey - containsWidth1_2, ex - midx,
                             containsWidth);
          chartText.textType2->setGeometry(
              midx - (chartText.textType2->width() >> 1),
              (height() >> 1) - (chartText.textType2->height() >> 1),
              chartText.textType2->width() + 30, chartText.textType2->height());
        } else {
          linePath.lineTo(ex, sy);
    ...
    case ORIENTION::NORTHEAST: {
      drawStraightLine(sx, sy, endPos.rx() + extendWidth45,
                       endPos.ry() - extendWidth45, linePath, *graphPath);
      drawLineHead(endDirect, endLineHeadType, endPos.rx(), endPos.ry(), p,
                   linePath, *graphPath);
    } break;
    case ORIENTION::NORTHWEST: {
  ...
  case ORIENTION::EAST: {
    ...
    case ORIENTION::EAST: {
      ...
      if (sx > ex) {
        ...
        graphPath->addRect(sx, ey - containsWidth1_2, ex - sx, containsWidth);
        chartText.textType2->setGeometry(
            sx - (chartText.textType2->width() >> 1),
            (height() >> 1) - (chartText.textType2->height() >> 1),
            chartText.textType2->width() + 30, chartText.textType2->height());
      } else {
        linePath.lineTo(ex, sy);
        ...
        graphPath->addRect(ex - containsWidth1_2, sy, containsWidth, ey - sy);
        chartText.textType2->setGeometry(
            ex - (chartText.textType2->width() >> 1),
            (height() >> 1) - (chartText.textType2->height() >> 1),
            chartText.textType2->width() + 30, chartText.textType2->height());
      }
      drawLineHead(endDirect, endLineHeadType, endPos.rx(), endPos.ry(), p,
    ...
    case ORIENTION::SOUTH: {
      ...
      if (sy > ey) {
        if (sx > ex) {
          ...
          linePath.lineTo(ex, sy);
          linePath.lineTo(ex, ey);
          graphPath->addRect(sx, sy + containsWidth1_2, ex - sx,
                             -containsWidth);
          graphPath->addRect(ex - containsWidth1_2, sy, containsWidth, ey - sy);
          chartText.textType2->setGeometry(
              (width() >> 1) - (chartText.textType2->width() >> 1),
  ...
  case ORIENTION::NORTHEAST: {
    ...
    case ORIENTION::NORTHEAST: {
      drawStraightLine(sx, sy, ex + extendWidth45, ey - extendWidth45, linePath,
                       *graphPath);
      drawLineHead(endDirect, endLineHeadType, ex, ey, p, linePath, *graphPath);
    } break;
    case ORIENTION::NORTHWEST: {
...
QDataStream &operator<<(QDataStream &fout, const Chart_Line &cl) {
  ...
  fout.writeRawData(reinterpret_cast<const char *>(&cl.startDirect),
                    sizeof(ORIENTION));
  fout.writeRawData(reinterpret_cast<const char *>(&cl.endDirect),
                    sizeof(ORIENTION));
  fout.writeRawData(reinterpret_cast<const char *>(&cl.startMagIndex),
                    sizeof(int));
  fout.writeRawData(reinterpret_cast<const char *>(&cl.endMagIndex),
                    sizeof(int));
  fout.writeRawData(reinterpret_cast<const char *>(&cl.startLineHeadType),
                    sizeof(LINE_HEAD_TYPE));
  fout.writeRawData(reinterpret_cast<const char *>(&cl.endLineHeadType),
                    sizeof(LINE_HEAD_TYPE));
  if (cl.startChartMag)
    fout.writeRawData(
        reinterpret_cast<const char *>(&cl.startChartMag->getID()),
        sizeof(int));
  else {
    int i = -1;
  ...
  if (cl.endChartMag)
    fout.writeRawData(reinterpret_cast<const char *>(&cl.endChartMag->getID()),
                      sizeof(int));
  else {
    int i = -1;
    fout.writeRawData(reinterpret_cast<const char *>(&i), sizeof(int));
...
QDataStream &operator>>(QDataStream &fin, Chart_Line &cl) {
  ...
  fin.readRawData(reinterpret_cast<char *>(&cl.startLineHeadType),
                  sizeof(LINE_HEAD_TYPE));
  fin.readRawData(reinterpret_cast<char *>(&cl.endLineHeadType),
                  sizeof(LINE_HEAD_TYPE));
