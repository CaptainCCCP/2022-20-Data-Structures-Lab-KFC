...
Chart_Base::Chart_Base(QWidget *parent, PaintChartType type, bool textType,
                       bool mov, int mpc, int spc)
    : QWidget(parent), magPoint(mpc), sizePoint(spc), movable(mov),
      chartText(textType) {
  chartType = type;
  widgetPosInit();
  varInit();
...
Chart_Base::Chart_Base(int x, int y, int w, int h, QWidget *parent,
                       PaintChartType type)
    : QWidget(parent), magPoint(4), sizePoint(4) {
  chartType = type;
...
void Chart_Base::paintInit() { updatePaintInfo(); }
...
void Chart_Base::pointInit() {
  ...
  for (auto it = sizePoint.i_point.begin(); it != sizePoint.i_point.end();
       it++) {
    (*it) = new i_pointbase;
    (*it)->i_pos = new QPoint;
  }
...
void Chart_Base::textInit() {
  ...
  chartText.textType2->appendPlainText("");//初始输入文本
  chartText.textType2->setFrameShape(QFrame::NoFrame);
  chartText.textType2->setFrameShadow(QFrame::Plain);
  chartText.textType2->move(paintStart.rx() + sizePointWidth,
                            paintStart.ry() + sizePointWidth);
  chartText.textType2->setStyleSheet("background:transparent;");
  chartText.textType2->setSizePolicy(QSizePolicy::Preferred,
                                     QSizePolicy::Preferred);
  chartText.textType2->adjustSize();
  chartText.textType2->setSizeAdjustPolicy(
  ...
  connect(chartText.textType2,
          SIGNAL(setTypeChangeTextPos(CHART_TEXT_MOUSE_TYPE, int, int)), this,
          SLOT(setTypeChangeTextPos(CHART_TEXT_MOUSE_TYPE, int, int)));
...
void Chart_Base::adjustPointInfo() {
  for (auto it = magPoint.i_point.begin(); it != magPoint.i_point.end(); it++) {
    ...
    case ORIENTION::NORTH:
      (*it)->setX((*it)->getX() + pointLineWidth1_2);
      break;
    case ORIENTION::NORTHEAST:
      (*it)->setX((*it)->getX() + pointLineWidth);
      break;
    case ORIENTION::EAST:
      (*it)->setX((*it)->getX() + pointLineWidth);
      (*it)->setY((*it)->getY() + pointLineWidth1_2);
      break;
    case ORIENTION::SOUTHEAST:
      (*it)->setX((*it)->getX() + pointLineWidth);
      (*it)->setY((*it)->getY() + pointLineWidth);
      break;
...
void Chart_Base::updateMagPointPath() {
  for (auto it = magPoint.i_point.begin(), end = magPoint.i_point.end();
       it != end; it++) {
    (*it)->newPath()->addEllipse((*it)->getX() - sizePointWidth1_2,
                                 (*it)->getY() - sizePointWidth1_2,
                                 sizePointWidth, sizePointWidth);
  }
}

void Chart_Base::updateSizePointPath() {
  for (auto it = sizePoint.i_point.begin(), end = sizePoint.i_point.end();
       it != end; it++) {
    (*it)->newPath()->addRect((*it)->getX() - sizePointWidth1_2,
                              (*it)->getY() - sizePointWidth1_2, sizePointWidth,
                              sizePointWidth);
  }
}
void Chart_Base::updateMagPointLine() {
  for (auto it = magPoint.i_point.begin(), end = magPoint.i_point.end();
       it != end; it++) {
    for (auto l_it = (*it)->i_lineStart.begin(),
              l_end = (*it)->i_lineStart.end();
         l_it != l_end; l_it++) {
      if ((*l_it)->widgetStart.rx() != (*it)->getX() + x() ||
          (*l_it)->widgetStart.ry() != (*it)->getY() + y()) {
    ...
    for (auto l_it = (*it)->i_lineEnd.begin(), l_end = (*it)->i_lineEnd.end();
         l_it != l_end; l_it++) {
      if ((*l_it)->widgetEnd.rx() != (*it)->getX() + x() ||
          (*l_it)->widgetEnd.ry() != (*it)->getY() + y()) {
...
void Chart_Base::updateTextInfo() {
  ...
  chartText.textType2->setMinimumWidth(w - 10);
  chartText.textType2->setMaximumHeight(w);
  chartText.textType2->setMinimumHeight(w - 10);
  chartText.textType2->adjustSize();
  if (chartText.textType2->x() + chartText.textType2->width() > paintEnd.rx()) {
    if (paintEnd.rx() - chartText.textType2->width() < paintStart.rx())
      ...
      chartText.textType2->move(paintEnd.rx() - chartText.textType2->width(),
                                chartText.textType2->y());
  }
  if (chartText.textType2->y() + chartText.textType2->height() >
      paintEnd.ry()) {
    if (paintEnd.ry() - chartText.textType2->height() < paintStart.ry())
      chartText.textType2->move(chartText.textType2->x(), paintStart.ry());
    else
      chartText.textType2->move(chartText.textType2->x(),
                                paintEnd.ry() - chartText.textType2->height());
  }
}
...
void Chart_Base::paintMagPoint(QPainter &p) {
  ...
  for (auto it = magPoint.i_point.begin(), end = magPoint.i_point.end();
       it != end; it++) {
#if (RUNTIME_PAINTMAG == ENABLE)
    path.addEllipse((*it).i_pos.rx() - magPointWidth1_2,
                    (*it).i_pos.ry() - magPointWidth1_2, magPointWidth,
                    magPointWidth);
#else
    tmpPath = (*it)->getPath();
    p.drawPath(*tmpPath);
...
void Chart_Base::paintSizePoint(QPainter &p) {
  ...
  for (auto it = sizePoint.i_point.begin(), end = sizePoint.i_point.end();
       it != end; it++) {
#if (RUNTIME_PAINTSIZE == ENABLE)
    path.addRect((*it).i_pos.rx() - sizePointWidth1_2,
                 (*it).i_pos.ry() - sizePointWidth1_2, sizePointWidth,
                 sizePointWidth);
#else
    tmpPath = (*it)->getPath();
    p.drawPath(*tmpPath);
...
void Chart_Base::paintSizeEdge(QPainter &p) {
  ...
  int sx = paintStart.rx(), sy = paintStart.ry(), ex = paintEnd.rx(),
      ey = paintEnd.ry();
  int /*sx1 = sx-sizePointWidth1_2,*/ sx2 = sx + sizePointWidth1_2,
                                      /*sy1 = sy-sizePointWidth1_2,*/ sy2 =
                                          sy + sizePointWidth1_2;
  int ex1 = ex - sizePointWidth1_2 /*,ex2 = ex+sizePointWidth1_2*/,
      ey1 = ey - sizePointWidth1_2 /*,ey2 = ey+sizePointWidth1_2*/;
...
bool Chart_Base::autoSetMagi(int &abcx, int &abcy, int &index) {
  ...
  if (mx >= x() && my >= y() && mx - x() <= width() && my - y() <= width()) {
    ...
    for (auto it = magPoint.i_point.begin(), end = magPoint.i_point.end();
      ...
      if (abs(x1 - mx) <= magPointAutoMagiRange &&
          abs(y1 - my) <= magPointAutoMagiRange) {
        abcx = x() + (*it)->getX();
        abcy = y() + (*it)->getY();
        index = i;
        return true;
...
void Chart_Base::saveStaticValue(QDataStream &fout) {
  fout.writeRawData(reinterpret_cast<const char *>(&Chart_Base::magPointWidth),
                    sizeof(int));
  fout.writeRawData(reinterpret_cast<const char *>(&Chart_Base::sizePointWidth),
                    sizeof(int));
  fout.writeRawData(reinterpret_cast<const char *>(&Chart_Base::pointLineWidth),
                    sizeof(int));
  qDebug() << magPointWidth << " " << sizePointWidth << " " << pointLineWidth
           << " " << chartIDCount;
}
void Chart_Base::loadStaticValue(QDataStream &fin) {
  fin.readRawData(reinterpret_cast<char *>(&Chart_Base::magPointWidth),
                  sizeof(int));
  fin.readRawData(reinterpret_cast<char *>(&Chart_Base::sizePointWidth),
                  sizeof(int));
  fin.readRawData(reinterpret_cast<char *>(&Chart_Base::pointLineWidth),
                  sizeof(int));
  updatePaddingInfo();
  qDebug() << "static value:" << magPointWidth << " " << sizePointWidth << " "
           << pointLineWidth << " " << chartIDCount;
}
QDataStream &operator<<(QDataStream &fout, const Chart_Base &cb) {
  fout.writeRawData(reinterpret_cast<const char *>(&cb.chartType),
                    sizeof(PaintChartType));
  fout.writeRawData(reinterpret_cast<const char *>(&cb.ID), sizeof(int));
  fout << cb.chartText;
  fout << cb.paintStart << cb.paintEnd << cb.widgetStart << cb.widgetEnd
       << cb.paintChartDrawPen << cb.paintChartFillPen;
  fout.writeRawData(reinterpret_cast<const char *>(&cb.movable), sizeof(bool));
  // qDebug() << "Chart Base Info:"
  //          << "start_point: " << cb.paintStart << cb.paintEnd <<
  //          cb.widgetStart
  //          << cb.widgetEnd;
  return fout;
}
QDataStream &operator>>(QDataStream &fin, Chart_Base &cb) {
  ...
  fin >> cb.chartText;
  fin >> cb.paintStart >> cb.paintEnd >> cb.widgetStart >> cb.widgetEnd >>
      cb.paintChartDrawPen >> cb.paintChartFillPen;
  fin.readRawData(reinterpret_cast<char *>(&cb.movable), sizeof(bool));
  // qDebug() << "input Chart Base Info:"
  //          << "start point: " << cb.paintStart << cb.paintEnd <<
  //          cb.widgetStart
  //          << cb.widgetEnd;
...
void Chart_Base::mousePressEvent(QMouseEvent *event) {
  {
    ...
    if (showAll && inSizePath(event->pos(), direct)) {
      ...
      emit sendThisClass(this, event->pos().rx() - borderWidth,
                         event->pos().ry() - borderWidth);
      curFlag = MOUSE_EVENT_TYPE::RUNTIME_CHANGE_POS;
      curIndex = direct;
      raise();
    } else {
...
void Chart_Base::mouseMoveEvent(QMouseEvent *event) {
  if (curFlag == MOUSE_EVENT_TYPE::NONE) {
    ...
    if (chartText.textMouseT1 == CHART_LABEL_MOUSE_TYPE::CHANGE_POS) {
      emit sendThisClass(this, event->pos().rx() - borderWidth,
                         event->pos().ry() - borderWidth);
      chartText.textType2->move(
          event->pos().rx() - chartText.chartTextMousePos.rx(),
          event->pos().ry() - chartText.chartTextMousePos.ry());
    }
  } else {
    event->ignore();
  }
...
void Chart_Base::mouseDoubleClickEvent(QMouseEvent *event) {
  ...
  emit sendThisClass(this, event->pos().rx() - borderWidth,
                     event->pos().ry() - borderWidth);
  chartText.tmpEdit2 =
      new QPlainTextEdit(chartText.textType2->toPlainText(), this);
  chartText.textType2->setPlainText("");
  chartText.tmpEdit2->adjustSize();
  chartText.tmpEdit2->setStyleSheet("background:transparent;");
  chartText.tmpEdit2->setGeometry(
      paintStart.rx() + textBorderWidth, paintStart.ry() + textBorderWidth,
      paintEnd.rx() - paintStart.rx() - (textBorderWidth << 1),
      paintEnd.ry() - paintStart.ry() - (textBorderWidth << 1));
  chartText.tmpEdit2->show();
  chartText.tmpEdit2->setFocus();
  chartText.tmpEdit2->grabKeyboard();
  // this->grabKeyboard();
  // } else {
  //
  //   chartText.tmpEdit2 =
  ...
  //   chartText.tmpEdit2->adjustSize();
  //   chartText.tmpEdit2->setStyleSheet("background:transparent;");
  //   chartText.tmpEdit2->setGeometry(paintStart.rx(), paintStart.ry(),
  //                                   paintStart.rx() + 100,
  //                                   paintStart.ry() + 100);
  //   chartText.tmpEdit2->show();
  //   chartText.tmpEdit2->setFocus();
